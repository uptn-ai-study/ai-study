...
| 항목 | 기준 |
|---|---|
| 화면 기준 너비 | 390px (iPhone 14 기준) |
| 터치 영역 | 최소 44px × 44px 이상 |
| Safe Area | 상단(status bar) / 하단(home indicator) 대응 필수 |
...
```html
<!-- index.html meta 태그 필수 -->
<meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
```

### 상태 처리 — 반드시 고려할 3가지

모든 화면은 아래 세 가지 상태를 설계 단계부터 고려해주세요.

| 상태 | 설명 | 예시 |
|---|---|---|
| 로딩 상태 | 데이터를 불러오는 중 | 스켈레톤 UI, 스피너 |
| 빈 상태 | 데이터가 없을 때 | "아직 항목이 없어요" 안내 문구 + CTA |
| 에러 상태 | 요청 실패 / 네트워크 오류 | 에러 메시지 + 재시도 버튼 |

```vue
<!-- 예시 구조 -->
<template>
  <div v-if="isLoading"><!-- 로딩 --></div>
  <div v-else-if="error"><!-- 에러 --></div>
  <div v-else-if="items.length === 0"><!-- 빈 상태 --></div>
  <div v-else><!-- 정상 데이터 --></div>
</template>
```
...
- 본문 최소 폰트 크기: **14px** (그 이하는 가독성 저하)
- `font-size: 16px` 미만 input → iOS에서 자동 zoom 발생 → input은 16px 이상 권장
- `word-break: keep-all` — 한국어 줄바꿈이 어색하게 끊기는 것 방지

### 터치 & 인터랙션

- 버튼, 링크 등 터치 요소의 `min-height` / `min-width` 는 **44px 이상**
- `:hover` 대신 `:active` 스타일 위주로 작성 (모바일은 hover 없음)
- `cursor: pointer` 는 모바일에서 의미 없으나 데스크탑 겸용이면 유지
- 스크롤 가능한 영역엔 `-webkit-overflow-scrolling: touch` 또는 `overflow: auto` 적용

### 레이아웃

- `100vw` 사용 시 스크롤바 너비 문제 발생 가능 → `width: 100%` 권장
- `100vh` 는 모바일 브라우저 주소창 높이 문제 있음 → `100dvh` 사용 권장
- flexbox / grid 사용 시 `min-width: 0` 없으면 overflow 발생할 수 있음

```css
/* 모바일 뷰포트 높이 */
height: 100dvh; /* 동적 뷰포트 높이 — 주소창 포함/제외 자동 대응 */
```

### 이미지

- 이미지엔 항상 `width`와 `height` 속성 지정 → CLS(레이아웃 밀림) 방지
- `loading="lazy"` — 스크롤 밖 이미지는 지연 로딩
- WebP 포맷 우선 사용 (용량 절감)

### 성능

- 리스트가 길어질 경우 가상 스크롤(virtual scroll) 고려
- 불필요한 `watch` / `computed` 남발 주의 — 모바일은 CPU 성능이 낮음
- API 호출은 중복 요청 방지 (`AbortController` 활용)

### 접근성 (기본)

- 이미지 `alt` 속성 필수
- 폼 input에 `label` 또는 `aria-label` 연결
